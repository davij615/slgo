// api/aggressive.js — CHALLENGER live: research-backed momentum ranking.
// Eligibility: listed exchanges (no OTC), $10+, $1B+ cap, $20M+ ADV, uptrend
// stack (close > SMA50 > SMA200), near 52w high, strong 1M/3M, Buy+ rating, ROE>0.
//   ?method=raw|mom121|riskadj|composite (default)   ?n=5 (default) or 10
...
const COLUMNS = [
  ...
  "average_volume_30d_calc", "exchange", "sector", "return_on_equity",
];
const MIN_DOLLAR_VOLUME = 20_000_000;
const HIGH_PROXIMITY = 0.75;
...
const FILTERS = [
  ...
  { left: "Perf.3M", operation: "egreater", right: 20 },
  { left: "Recommend.All", operation: "egreater", right: 0.1 },
  { left: "ADX", operation: "egreater", right: 25 },
...
export default async function handler(req, res) {
  const n = Math.min(Math.max(parseInt(req.query?.n) || 5, 1), 25);
  const method = ["raw", "mom121", "riskadj", "composite"].includes(req.query?.method)
    ? req.query.method : "composite";
  const full = req.query?.full === "1";   // all eligible (for balanced book)
  try {
    ...
    rows = rows
      ...
      .filter((r) => r.return_on_equity == null || r.return_on_equity > 0);

    scoreRows(rows, method);
    rows.sort((a, b) => (b.score ?? -1e9) - (a.score ?? -1e9));
    const top = (full ? rows.slice(0, 150) : rows.slice(0, n)).map((r) => ({
      ...r, score: r.score == null ? null : +r.score.toFixed(3),
      mom121: r.mom121 == null ? null : +(r.mom121 * 100).toFixed(1),
    }));
    res.setHeader("Cache-Control", "s-maxage=3600, stale-while-revalidate=86400");
    res.status(200).json({
      asOf: new Date().toISOString(), strategy: `aggressive-${method}${full ? "-full" : `-top${n}`}`,
      method, full, total: rows.length, count: top.length, rows: top,
    });
  } catch (e) {
